...
class VectorCodemaster(Codemaster):
    ...
    def set_game_state(self, words_on_board: List[str], key_grid: List[str]) -> None:
        ...
        good_words, bad_words = self._identify_words_on_board()
        self._calc_distance_between_words_on_board_and_clue(good_words, bad_words)
        self._remove_conflicting_clues(good_words, bad_words)

    def _calc_distance_between_words_on_board_and_clue(self, good_words: List[str], bad_words: List[str]) -> None:
        """Create word-distance dictionaries for both good words and bad words"""
        self.good_word_distances = {}
        for goodWord in good_words:
            self.good_word_distances[goodWord] = {}
            good_word_stacked = self._hstack_word_vectors(goodWord)
            for potentialClue in self.cm_word_set | set(bad_words) | set(good_words):
                try:
                    dist = scipy.spatial.distance.cosine(good_word_stacked, self._hstack_word_vectors(potentialClue))
                except KeyError:
                    dist = np.inf
                self.good_word_distances[goodWord][potentialClue] = dist
    ...
    def _remove_conflicting_clues(self, good_words: List[str], bad_words: List[str]) -> None:
        """Remove and save clues that overlap with words on the board"""
        self.removed_clues = {}
        for word in bad_words + good_words:
    ...
    def _identify_words_on_board(self) -> Tuple[List[str], List[str]]:
        ...
        for i in range(len(self.key_grid)):
            ...
            if self.words_on_board[i][0] == '*':
                continue
            elif self.key_grid[i] == "Assassin" or self.key_grid[i] == self.bad_color or self.key_grid[i] == "Civilian":
                bad_words.append(self.words_on_board[i].lower())
            else:
                good_words.append(self.words_on_board[i].lower())
    ...
    def get_clue(self) -> Tuple[str, int]:
        ...
        good_words, bad_words = self._identify_words_on_board()
        print(f"{self.color.upper()} :", good_words)
        ...
        to_remove = set(self.good_word_distances) - set(good_words)
        for word in to_remove:
            # del self.goodWordDists[word]
            removed_clues_per_word = self.removed_clues.get(word, [])
            for clue in removed_clues_per_word:
                self.cm_word_set.add(clue)

        bests = {}
        # iterate though combinations of good words for best clue
        # ignore clue with close distance to a bad word
        for n_good_words_in_clue in range(1, self.max_good_words_per_clue + 1):
            best_dist_per_clue_num = np.inf
            best_word_per_clue_num = ""
            best_good_word_per_clue_num = ""

            for good_word_combination in list(itertools.combinations(good_words, n_good_words_in_clue)):
                best_dist = np.inf
                best_word = ""
                ...
                for potential_clue in self.cm_word_set:
                    ...
                    max_good_dist = 0
                    for good_word in good_word_combination:
                        dist = self.good_word_distances[good_word][potential_clue]
                        if dist > max_good_dist:
                            max_good_dist = dist

                    if max_good_dist < best_dist and max_good_dist < min_bad_dist:
                        best_dist = max_good_dist
                        ...
                        if best_dist < best_dist_per_clue_num:
                            ...
                            best_good_word_per_clue_num = good_word_combination

            bests[n_good_words_in_clue] = (best_good_word_per_clue_num, best_word_per_clue_num, best_dist_per_clue_num)
        ...
        for clue_num, (goodWordCombo, potential_clue, score) in bests.items():
            if score == np.inf:
                continue
            ...
            for word in goodWordCombo:
                dist = self.good_word_distances[word][potential_clue]
                if dist > worst:
                    worst = dist
                if dist < best:
                    best = dist
            ...
            # where the worst distance between clue and goodWord is below distanceThreshold

...
class AICodemaster(Codemaster):
    ...
    def get_clue(self):
        ...
        # Creates Blue-Labeled Word arrays, and everything else arrays
        for i in range(25):
            if self.words[i][0] == '*':
                continue
        ...
        if not self.bad_word_dists:
            ...
            self.blue_word_dists = {}
            for word in blue_words:
                self.blue_word_dists[word] = {}
                for val in self.cm_wordlist:
                    b_dist = cos_dist(self.concatenate(val, all_vectors), self.concatenate(word, all_vectors))
                    self.blue_word_dists[word][val] = b_dist
        ...
        for clue_num in range(1, 3 + 1):
            ...
            for blue_word in list(itertools.combinations(blue_words, clue_num)):
                ...
                for word in self.cm_wordlist:
                    if not self.arr_not_in_word(word, blue_words + bad_words):
                        continue
                    ...
                    for bad_word in self.bad_word_dists:
                        if self.bad_word_dists[bad_word][word] < bad_dist:
                            ...
                            worst_bad = bad_word
                    worst_blue = 0
                    for blue in blue_word:
                        dist = self.blue_word_dists[blue][word]
                        if dist > worst_blue:
                            worst_blue = dist

                    if worst_blue < best_dist and worst_blue < bad_dist:
                        best_dist = worst_blue
                        best_word = word
                        # print(worst_blue,blue_word,word)

                        if best_dist < best_per_dist:
                            best_per_dist = best_dist
                            best_per = best_word
                            best_blue_word = blue_word
            bests[clue_num] = (best_blue_word, best_per, best_per_dist)
        ...
        for clue_num, clue in bests.items():
            best_blue_word, combined_clue, combined_score = clue
            worst = -np.inf
            best = np.inf
            worst_word = ''
            for word in best_blue_word:
            ...
            li.append((worst / best, best_blue_word, worst_word, combined_clue,
                       combined_score, combined_score ** len(best_blue_word)))
